...
contract web3wiki {
   ...
   constructor(){
    ...
    function createarticle(string memory uri) public{   //new article
       articles.push(uri);
       writers_articles[msg.sender].push(articles.length-1);
       ownershare[articles.length-1][msg.sender]=100;
    }
    function updatearticle(uint256 Id,string memory uri) internal{      //update content in article
        articles[Id]=uri;
    }

    function ownershiptransfer(uint256 articleId,address transfer_to,uint8 amount)public {  //transfering an article ownership
        require((articles.length-1)>=articleId,"article doesn't exist");
        require(ownershare[articleId][msg.sender]>=amount,"you not have required share");
    ...
    struct request {      //requested changes
        address owner;
        uint256 articleId;
        string uri;
    ...
    function voterequest(uint256 requestId)public {                     //upvoting for an update request
        ...
        if(requests[requestId].acceptedamount>=65){
            ...
            uint256[] memory requestlist=articles_to_request[requests[requestId].articleId];
            for(uint i=0;requestlist.length>i;i++){
             if(requestlist[i]==requestId){
                 articles_to_request[requests[requestId].articleId][i]=articles_to_request[requests[requestId].articleId][requestlist.length-1];
    ...
    function requestdata(uint256 requesetId)public view returns(request memory){
        return requests[requesetId];
    }
    function request_of_articleId(uint256 articleid)public view returns(uint256[] memory){
